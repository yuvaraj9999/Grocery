...
namespace SmartStore.Web.Controllers
{
    public partial class HomeController : PublicControllerBase
    {
        ...
        [ChildActionOnly]
        [HttpGet]
        public ActionResult HomepageImageSlider()
        {
            List<ImageSlider> sliderimages = new List<ImageSlider>();
            string CS = ConfigurationManager.ConnectionStrings["SliderConnection"].ConnectionString;
            using (SqlConnection con = new SqlConnection(CS))
            {
                ...
                while (rdr.Read())
                {
                    ...
                    slider.IName = rdr["IName"].ToString();
                    if (rdr["Image"] != null && rdr["Image"] != DBNull.Value)
                    {
                        slider.Image = (byte[])rdr["Image"];
                    }
                    slider.FilePath = rdr["FilePath"].ToString();
                    slider.MobileImageFilePath = rdr["MobileImageFilePath"].ToString();
                    if (rdr["MobileImage"] != null && rdr["MobileImage"] != DBNull.Value)
                    {
                        slider.MobileImage = (byte[])rdr["MobileImage"];
                    }
